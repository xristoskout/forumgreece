  ...
  return (
    <div className="max-w-6xl mx-auto p-3 sm:p-6 space-y-8 sm:space-y-12 bg-transparent">
      ...
      <div className="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 text-slate-800">Trip Parameters</h3>
            ...
            <div key={d.id} className="border p-4 sm:p-8 rounded-3xl bg-white shadow-sm flex flex-col justify-between">
                ...
                <div className="space-y-3 border-t pt-6">
                    <div className="flex justify-between items-center text-sm gap-2">
                      ...
                      <span className="font-bold whitespace-nowrap">{sym}{Math.round(cost.hotel)}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm gap-2">
                      <span className="text-slate-600">Food & Drink</span> 
                      <span className="font-bold whitespace-nowrap">{sym}{Math.round(cost.food)}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm gap-2">
                      <span className="text-slate-600">Transport</span> 
                      <span className="font-bold whitespace-nowrap">{sym}{Math.round(cost.trans)}</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-4 text-xs text-slate-500 pt-2 justify-center border-t border-slate-50 mt-4">
                      <div className="flex items-center gap-1.5 whitespace-nowrap">
                        <span className="w-2.5 h-2.5 rounded-full bg-indigo-500"></span> Accommodation
                      </div>
                      <div className="flex items-center gap-1.5 whitespace-nowrap">
                        <span className="w-2.5 h-2.5 rounded-full bg-teal-500"></span> Food
                      </div>
                      <div className="flex items-center gap-1.5 whitespace-nowrap">
                        <span className="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Transport
                      </div>
                    </div>
                </div>
            </div>
      ...
      {/* 3. Comparison Matrix */}
      <div className="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Comparison Matrix</h2>
        <div className="overflow-x-auto -mx-4 sm:mx-0">
            <table className="w-full text-left text-sm">
            <thead>
                <tr className="border-b">
                  <th className="p-2 sm:p-4 text-left">Feature</th>
            ...
            <tbody>
                ...
                <tr className="border-b font-semibold text-indigo-900 bg-indigo-50/30">
                    <td className="p-2 sm:p-4">Avg Temp ({month})</td>
                    <td className="p-2 sm:p-4 text-center">{s1.seasonality[month]?.temp ?? 'N/A'}°C</td>
                    <td className="p-2 sm:p-4 text-center">{s2.seasonality[month]?.temp ?? 'N/A'}°C</td>
                </tr>
                {['family', 'couples', 'nightlife', 'food', 'beaches', 'walking'].map(key => (
                    <tr key={key} className="border-b">
                        <td className="p-2 sm:p-4 capitalize font-semibold">{key}</td>
                        <td className="p-2 sm:p-4 text-center">{s1.scores[key]?.value ?? 0}/10</td>
                        <td className="p-2 sm:p-4 text-center">{s2.scores[key]?.value ?? 0}/10</td>
                    </tr>
                ))}
            </tbody>
            </table>
      ...
      <div className="bg-indigo-900 text-white p-4 sm:p-8 rounded-3xl mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="text-center md:text-left">
            <h3 className="text-xl sm:text-2xl font-bold">Recommendation</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 mt-8">
        {[s1, s2].map(d => (
            <div key={d.id} className="border p-4 sm:p-8 rounded-3xl bg-white shadow-sm">
                <h4 className="font-bold text-base sm:text-lg mb-4 text-indigo-900">Highlights of {d.name}</h4>
