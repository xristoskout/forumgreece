...
import { food, Lang, siteBrand, siteBrandLine } from "../../../lib/content";

export default function EatDrinkDetailPage() {
  const [lang, setLang] = useState<Lang>("en");
  const params = useParams<{ slug: string }>();
  const item = food.find((entry) => entry.slug === params.slug);

  const t = {
    navHome: { en: "Home", el: "Αρχική" },
    navSection: { en: "Eat & Drink", el: "Φαγητό & Ποτό" },
    eyebrow: { en: "Food & Drink Guide", el: "Οδηγός Φαγητού & Ποτού" },
    overview: { en: "Overview", el: "Επισκόπηση" },
    specialties: { en: "What stands out", el: "Τι ξεχωρίζει" },
    place: { en: "Place", el: "Περιοχή" },
    back: { en: "Back to homepage", el: "Επιστροφή στην αρχική" },
    notFound: { en: "Food page not found.", el: "Η σελίδα φαγητού δεν βρέθηκε." },
  };
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <ul className="mt-4 space-y-3 text-slate-600">
            {item.specialties[lang].map((specialty) => (
              <li key={specialty} className="rounded-2xl bg-slate-50 px-4 py-3">
                {specialty}
              </li>
            ))}
          </ul>
