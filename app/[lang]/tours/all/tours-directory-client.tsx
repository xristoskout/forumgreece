...
const regions = [
  ...
  { key: "Rhodes", label: { en: "Rhodes", el: "Ρόδος" }, emoji: "🛡️", image: "/images/rhodes.webp", count: 130, description: { en: "Medieval City, Lindos, Prasonisi.", el: "Μεσαιωνική Πόλη, Λίνδος, Πρασονήσι." }, highlights: { en: ["Medieval City", "Lindos Acropolis", "Prasonisi surfing"], el: ["Παλιά Πόλη Ρόδου", "Ακρόπολη Λίνδου", "Πρασονήσι"] }, landingSlug: "rhodes-tours", destinationSlug: "rhodes" },
      ...
      <section className="mx-auto max-w-7xl px-6 mt-20">
        ...
        <div className="flex flex-wrap justify-center gap-2.5 max-w-4xl mx-auto">
          {regions.map(region => {
            ...
            const pillClass = `group relative px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 ${
              isActive
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 mt-12">
          {regions.map(region => {
            const isActive = selectedRegion === region.key;
            const hasWidget = !!VIATOR_WIDGET_MAPPING[region.key];
            const cardClass = `group relative overflow-hidden rounded-2xl border transition-all duration-300 text-left ${
              isActive
